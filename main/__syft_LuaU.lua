...
local TB=38; local IH=28; local BH=30; local SH=42; local DIH=24; local PAD=12; local CP=8
...
function SyftLib:Tab(name)
    ...
    function tab:Section(title)
        ...
        function sec:Button(lbl,cb) table.insert(self.items,{kind="btn",label=lbl,cb=cb,_hc=nil}) end
        function sec:Dropdown(lbl,opts,cb) table.insert(self.items,{kind="dd",label=lbl,opts=opts,sel=opts[1] or "",cb=cb,open=false,_hc=nil}) end
        function sec:MultiDropdown(lbl,opts,cb) table.insert(self.items,{kind="mdd",label=lbl,opts=opts,sel={},cb=cb,open=false,_hc=nil}) end
...
function SyftLib:Open()
    ...
    local srW=140; local srBg,srBrd,srTxt
    ...
    local cH=lib.sh-TB-CP*2; local SCSP=22
    ...
    local function buildSecs()
        ...
        local leftIdx=0; local rightIdx=0
        for _,sec in ipairs(cur.sections) do
            if secVis(sec,q) then
                local ch=secH(sec,q)
                if ch>0 then
                    leftIdx=leftIdx+1
                    local isLeft=(leftIdx%2==1)
                    if not isLeft then rightIdx=rightIdx+1 end
        ...
        local clipT=lib.py+TB; local clipB=lib.py+lib.sh-CP
        ...
        for _,L in ipairs(layouts) do
            ...
            if cy+ch>=clipT and cy<=clipB then
                ...
                for _,it in ipairs(sec.items) do
                    ...
                    local vis=(not q or q=="") or nm:find(q:lower(),1,true)~=nil
                    if vis then
                        local inV=(iy>=clipT-40 and iy<=clipB+20)
                            ...
                            if it.tip and inV then
                                ...
                                sloop(function(h)
                                    ...
                                    if hov then
                                        ...
                                        ttBg.Position=Vector2.new(bx,by); ttBrd.Position=Vector2.new(bx,by); ttT.Position=Vector2.new(bx+10,by+5)
                        ...
                        elseif it.kind=="tog" then
                            ...
                            local cbBg=mk("Square",{Filled=true,Color=lerpC(C.surface1,C.mauve,it._at),Size=Vector2.new(16,16),Position=Vector2.new(cx+colW2-PAD-16,iy+6),Corner=4,ZIndex=13,Visible=true})
                            local cbBrd=mk("Square",{Filled=false,Color=lerpC(C.surface1,C.mauve,it._at),Size=Vector2.new(16,16),Position=Vector2.new(cx+colW2-PAD-16,iy+6),Corner=4,Thickness=1,ZIndex=14,Visible=true})
                            local bx=cx+colW2-PAD-16; local by=iy+6
                            local ck1=mk("Line",{})
                            ...
                            if inV then
                                ...
                                sloop(function(h)
                                    ...
                                    capIt._at=lerpN(capIt._at,tgt,0.22)
                                    local ac=lerpC(C.surface1,C.mauve,capIt._at)
                                    ...
                                    lblT.Color=hov and C.text or lerpC(C.subtext1,C.text,capIt._at)
                                    h.wd=d
                                end)
                            end
                            iy=iy+IH
                        ...
                        elseif it.kind=="sld" then
                            ...
                            local sTh=mk("Square",{Filled=true,Color=C.wht,Size=Vector2.new(15,15),Position=Vector2.new(it._tx,iy+21),Corner=8,ZIndex=15,Visible=true})
                            local sThI=mk("Square",{Filled=true,Color=C.mauve,Size=Vector2.new(7,7),Position=Vector2.new(it._tx+4,iy+25),Corner=4,ZIndex=16,Visible=true})
                            if inV then
                                local capTX=trkX; local capTW=trkW; local capCX=cx; local capCW=colW2; local capIY=iy
                                local targetX=trkX+fw-7
                                sloop(function(h)
                                    local d=ismouse1pressed()
                                    ...
                                    if h.drag then
                                        ...
                                        sF.Size=Vector2.new(fw2,5); targetX=capTX+fw2-7
                                    ...
                                    capIt._tx=lerpN(capIt._tx,targetX,0.35)
                                    sTh.Position=Vector2.new(capIt._tx,capIY+21)
                                    sThI.Position=Vector2.new(capIt._tx+4,capIY+25)
                        ...
                        elseif it.kind=="btn" then
                            ...
                            local capIt=it
                            local bBg =mk("Square",{Filled=true,Color=lerpC(C.surface0,C.surface2,it._hc),Size=Vector2.new(colW2-PAD*2,BH),Position=Vector2.new(cx+PAD,iy),Corner=6,ZIndex=13,Visible=true})
                            local bBrd=mk("Square",{Filled=false,Color=lerpC(C.brd,C.mauve,it._hc),Size=Vector2.new(colW2-PAD*2,BH),Position=Vector2.new(cx+PAD,iy),Corner=6,Thickness=1,ZIndex=14,Visible=true})
                            local lw=tw(it.label,FSS)
                            local bTxt=mk("Text",{Text=it.label,Size=FSS,Color=lerpC(C.subtext1,C.text,it._hc),Font=FONT,Position=Vector2.new(cx+PAD+math.floor((colW2-PAD*2-lw)/2),iy+8),ZIndex=15,Visible=true})
                            if inV then
                                sloop(function(h)
                                    local d=ismouse1pressed()
                                    local hov=over(bBg.Position,bBg.Size)
                                    local tgt=hov and 1 or 0
                                    capIt._hc=lerpN(capIt._hc,tgt,0.2)
                                    bBg.Color=lerpC(C.surface0,C.surface2,capIt._hc)
                                    bBrd.Color=lerpC(C.brd,C.mauve,capIt._hc)
                                    bTxt.Color=lerpC(C.subtext1,C.text,capIt._hc)
                        ...
                        elseif it.kind=="dd" or it.kind=="mdd" then
                            iy=iy+2
                            local isM=(it.kind=="mdd")
                            local sid={}
                            if it._hc==nil then it._hc=0 end
                            local capIt=it
                            local dBg =mk("Square",{Filled=true,Color=lerpC(C.surface0,C.surface1,it._hc),Size=Vector2.new(colW2-PAD*2,BH),Position=Vector2.new(cx+PAD,iy),Corner=6,ZIndex=13,Visible=true})
                            local dBrd=mk("Square",{Filled=false,Color=lerpC(C.brd,C.mauve,it._hc),Size=Vector2.new(colW2-PAD*2,BH),Position=Vector2.new(cx+PAD,iy),Corner=6,Thickness=1,ZIndex=14,Visible=true})
                            local selText
                            if isM then
                                local t2={}; for _,v in ipairs(it.sel) do table.insert(t2,v) end
                                selText=#t2==0 and "none selected" or table.concat(t2,", ")
                            else selText=it.sel end
                            ...
                            for oi,opt in ipairs(it.opts) do
                                ...
                                if isM then
                                    ...
                                    if isSel then
                                        mk("Square",{Filled=true,Color=C.mauve,Size=Vector2.new(3,DIH-8),Position=Vector2.new(cx+PAD+3,oY+4),Corner=2,ZIndex=22,Visible=false})
                                    end
                                end
                            ...
                            if inV then
                                sloop(function(h)
                                    ...
                                    local tgt=hov and 1 or 0
                                    capIt._hc=lerpN(capIt._hc,tgt,0.2)
                                    dBg.Color=lerpC(C.surface0,C.surface1,capIt._hc)
                                    dBrd.Color=lerpC(C.brd,C.mauve,capIt._hc)
                                    dTxt.Color=lerpC(C.subtext1,C.text,capIt._hc)
                                    if d and not h.wd then
                                        if hov then
                                    ...
                                    for oi,od in ipairs(oDs) do
                                        ...
                                        if capIt.open and d and not h.wd then
                                            if over(Vector2.new(lBg.Position.X,od.y),Vector2.new(lBg.Size.X,DIH)) then
                                                if isM then
                                                    local found=false
                                                    for idx,v in ipairs(capIt.sel) do
                                                        if v==od.v then table.remove(capIt.sel,idx); found=true; break end
                                                    end
                                                    ...
                                                    print("[MULTISELECT]",capIt.label,table.concat(t2,", "))
                                                else
        ...
        if q and q~="" and #layouts==0 then
            ...
            mk("Square",{Filled=true,Color=C.mantle,Size=Vector2.new(noW,32),Position=Vector2.new(lib.px+math.floor(lib.sw/2-noW/2),lib.py+TB+32),Corner=6,ZIndex=10,Visible=true})
            mk("Text",{Text="no results",Size=FSS,Color=C.overlay0,Font=FONT,Position=Vector2.new(lib.px+math.floor(lib.sw/2-tw("no results",FSS)/2),lib.py+TB+40),ZIndex=11,Visible=true})
        end
    end
    ...
    if lib.doSearch then
        spawn(function()
            ...
            while lib.visible do
                local sx2=lib.px+tw(lib.title,FS)+26
                if srBg then srBg.Position=Vector2.new(sx2,lib.py+8); srBrd.Position=srBg.Position; srTxt.Position=Vector2.new(sx2+8,lib.py+12) end
                local d=ismouse1pressed()
                ...
                if srTxt then srTxt.Text=lib.query=="" and "search..." or lib.query; srTxt.Color=lib.query=="" and C.overlay0 or C.text end
    ...
    spawn(function()
        ...
        while lib.visible do
            local d=ismouse1pressed()
            for i=1,numT do
                local tapX=lib.px+TITLEW+(i-1)*eTW; local tapY=lib.py
                if over(Vector2.new(tapX,tapY),Vector2.new(eTW,TB)) and d and not wd and not openDD and not lib.sfocus then
            ...
            slideRelXCur=lerpN(slideRelXCur,slideRelX,0.22)
            slideLine.Position=Vector2.new(lib.px+TITLEW+slideRelXCur,lib.py+TB-3)
            wd=d; wait(0.016)
        end
    end)
    ...
    spawn(function()
        while lib.visible do
            ...
            if inC then
                ...
                if cur2 then
                    local q=lib.query; local c1H=0; local c2H=0; local lIdx=0
                    for _,sec in ipairs(cur2.sections) do
                        if secVis(sec,q) then local ch=secH(sec,q)
                            if ch>0 then lIdx=lIdx+1; if lIdx%2==1 then c1H=c1H+ch+CP else c2H=c2H+ch+CP end end
                        end
                    end
                    local totH2=math.max(c1H,c2H); local maxS2=math.max(0,totH2-cH)
